...
class HouseSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for city_tag in city_tags:
            city_url = city_tag.css('::attr(href)').get() # attr 返回选择元素的属性值
    ...
    def parse_house_page(self, response):
        ...
        for x in range(1, totalPage + 1):
            yield scrapy.Request(response.url + "pg" + str(x), callback=self.parse_house_list,
    ...
    def parse_house_list(self, response):
        ...
        for detail_url in detail_urls:
            ...
            if result:
                yield scrapy.Request(detail_url, callback=self.parse_huose, meta={"item": item})

    def parse_huose(self, response):
        # 解析房源详情页
        item = response.meta.get('item')
        item['title'] = response.css("h1.main::text").get()
        print(response.css("h1.main::text").get())
